...
/// </summary>
public class FieldManager_Tests : SitesEntityFrameworkCoreTestBase
{
    ...
    public async Task Should_Move_Stored_Values_When_Renaming_A_Field()
    {
        ...
        {
            var field = await _fieldRepository.GetAsync(SitesTestData.ViewsFieldId);
    ...
    public async Task Should_Leave_Index_Rows_Intact_When_Renaming_A_Field()
    {
        var before = await WithUnitOfWorkAsync(() =>
            _indexRepository.GetListAsync(x => x.FieldId == SitesTestData.CategoryFieldId));

        await WithUnitOfWorkAsync(async () =>
        {
            var field = await _fieldRepository.GetAsync(SitesTestData.CategoryFieldId);
    ...
    public async Task Should_Reject_Rename_Onto_An_Existing_Name()
    {
        ...
        {
            var field = await _fieldRepository.GetAsync(SitesTestData.BodyFieldId);
