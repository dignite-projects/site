...
public class ContentTypeManager_Tests : SitesEntityFrameworkCoreTestBase
{
    ...
    public async Task Should_Persist_A_Changed_Field_Arrangement()
    {
        ...
        {
            var contentType = await _contentTypeRepository.GetAsync(SitesTestData.PostGalleryTypeId);

            await _contentTypeManager.UpdateAsync(
                contentType, contentType.Name, contentType.DisplayName, contentType.Description,
                new[]
                {
                    new ContentTypeField(SitesTestData.TitleFieldId, required: true, order: 0),
                    new ContentTypeField(SitesTestData.BodyFieldId, order: 1),
                    new ContentTypeField(SitesTestData.CategoryFieldId, searchable: true, order: 2)
                });
        });

        var reloaded = await WithUnitOfWorkAsync(() =>
            _contentTypeRepository.GetAsync(SitesTestData.PostGalleryTypeId));

        reloaded.Fields.Count.ShouldBe(3);
        reloaded.GetFieldIds().ShouldContain(SitesTestData.CategoryFieldId);
        reloaded.Fields.Single(f => f.FieldId == SitesTestData.TitleFieldId).Required.ShouldBeTrue();
    }
    ...
    public async Task Should_Reindex_Existing_Contents_When_A_Field_Becomes_Searchable()
    {
        ...
        var before = await WithUnitOfWorkAsync(() =>
            indexRepository.GetListAsync(x => x.FieldId == SitesTestData.BodyFieldId));
        before.ShouldBeEmpty();

        await WithUnitOfWorkAsync(async () =>
        {
            var contentType = await _contentTypeRepository.GetAsync(SitesTestData.PostArticleTypeId);
            ...
            await _contentTypeManager.UpdateAsync(
                ...
                contentType.Fields
                    .Select(f => new ContentTypeField(
                        ...
                        f.Required,
                        // the flip under test
                        searchable: f.FieldId == SitesTestData.BodyFieldId || f.Searchable,
                        f.ShowInList,
                        f.DisplayName,
                        f.Order))
                    .ToList());
    ...
    public async Task Should_Store_Fields_In_Order()
    {
        var contentType = await WithUnitOfWorkAsync(() => _contentTypeManager.CreateAsync(
            SitesTestData.BlogPageId, "ordered-type", "Ordered type",
            fields: new[]
            {
                new ContentTypeField(SitesTestData.BodyFieldId, order: 5),
                new ContentTypeField(SitesTestData.TitleFieldId, order: 1)
            }));

        contentType.Fields.Select(f => f.FieldId)
            .ShouldBe(new[] { SitesTestData.TitleFieldId, SitesTestData.BodyFieldId });
    }
    ...
    public async Task Should_Reject_A_Field_Referenced_Twice()
    {
        await Should.ThrowAsync<ContentTypeFieldDuplicatedException>(() => WithUnitOfWorkAsync(() =>
            _contentTypeManager.CreateAsync(
                SitesTestData.BlogPageId, "duplicated-field-type", "Duplicated",
                fields: new[]
                {
                    new ContentTypeField(SitesTestData.TitleFieldId, order: 0),
                    new ContentTypeField(SitesTestData.TitleFieldId, order: 1)
                })));
    }
    ...
    public async Task Should_Scope_Name_Uniqueness_To_The_Page()
    {
        await Should.ThrowAsync<ContentTypeNameAlreadyExistException>(() => WithUnitOfWorkAsync(() =>
            _contentTypeManager.CreateAsync(SitesTestData.BlogPageId, "post-article", "Clashes")));

        var onAnotherPage = await WithUnitOfWorkAsync(() =>
            _contentTypeManager.CreateAsync(SitesTestData.NewsPageId, "post-article", "Fine here"));
