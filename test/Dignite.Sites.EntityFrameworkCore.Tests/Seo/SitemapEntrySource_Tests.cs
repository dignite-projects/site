...
/// </summary>
public class SitemapEntrySource_Tests : SitesEntityFrameworkCoreTestBase
{
    ...
    {
        (await GetLocationsAsync()).ShouldNotContain($"{BaseUrl}/blog/{SitesTestData.DraftSlug}");
    }

    [Fact]
    public async Task Should_Not_Emit_Archived_Contents()
    {
        await CreateBlogPostAsync("archived-post", ContentStatus.Archived, SitesTestData.PublishTime);
    ...
    public async Task NoIndexing_A_Pages_Single_Content_Should_Remove_The_Page_Url()
    {
        ...
        {
            await _contentManager.CreateAsync(
                contentTypeId, SitesTestData.EnglishCulture, "", SitesTestData.PublishTime,
                ContentStatus.Published,
                new Dictionary<string, object?>
                {
                    ["title"] = "Legal",
    ...
    public async Task An_Inactive_Page_Should_Contribute_Nothing()
    {
        await WithUnitOfWorkAsync(async () =>
        {
            var blog = await GetRequiredService<IPageRepository>().GetAsync(SitesTestData.BlogPageId);
    ...
    private async Task CreateBlogPostAsync(string slug, ContentStatus status, DateTime publishTime)
    {
        ...
        {
            await _contentManager.CreateAsync(
                SitesTestData.PostArticleTypeId, SitesTestData.EnglishCulture, slug, publishTime, status,
    ...
    {
        return CreateSeoEnabledTypeAsync(SitesTestData.BlogPageId);
    }
