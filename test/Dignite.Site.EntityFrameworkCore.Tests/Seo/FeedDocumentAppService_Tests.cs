...
/// </summary>
public class FeedDocumentAppService_Tests : SiteEntityFrameworkCoreTestBase
{
    ...
    public async Task An_Seo_Meta_Title_Should_Win_Over_A_Text_Field()
    {
        ...
        await WithUnitOfWorkAsync(async () =>
        {
            await _contentManager.CreateAsync(
                contentTypeId, SiteTestData.EnglishCulture, "with-seo", SiteTestData.PublishTime,
    ...
    public async Task An_Seo_Meta_Title_Alone_Should_Not_Duplicate_Itself_As_The_Description()
    {
        ...
        {
            await _contentManager.CreateAsync(
                contentTypeId, SiteTestData.EnglishCulture, "seo-title-only", SiteTestData.PublishTime,
                ContentStatus.Published,
                new Dictionary<string, object?>
                {
                    ["title"] = "The field title",
    ...
    public async Task Drafts_Should_Not_Be_Syndicated()
    {
        ...
        document!.Content.ShouldNotContain(SiteTestData.DraftSlug);
    }
    ...
    public async Task Should_Page_Past_A_Run_Of_NoIndexed_Content_To_Reach_MaxItems()
    {
        ...
        await WithUnitOfWorkAsync(async () =>
        {
            ...
            {
                await _contentManager.CreateAsync(
                    contentTypeId, SiteTestData.EnglishCulture, $"hidden-{i}",
            ...
            {
                await _contentManager.CreateAsync(
                    contentTypeId, SiteTestData.EnglishCulture, $"visible-{i}",
    ...
    public async Task An_Inactive_Page_Should_Have_No_Feed()
    {
        await WithUnitOfWorkAsync(async () =>
        {
            ...
            var blog = await pageRepository.GetAsync(SiteTestData.BlogPageId);
    ...
    private async Task CreateBlogPostAsync(string slug, DateTime publishTime)
    {
        ...
        {
            await _contentManager.CreateAsync(
                SiteTestData.PostArticleTypeId, SiteTestData.EnglishCulture, slug, publishTime,
                ContentStatus.Published,
    ...
    private async Task<Guid> CreateSeoEnabledBlogTypeAsync()
    {
        return await WithUnitOfWorkAsync(async () =>
        {
            ...
            var contentType = await _contentTypeManager.CreateAsync(
                SiteTestData.BlogPageId, $"seo-{Guid.NewGuid():N}", "SEO type",
                fields: new[]
                {
    ...
    private async Task<Guid> CreateSeoEnabledBlogTypeWithBodyAsync()
    {
        return await WithUnitOfWorkAsync(async () =>
        {
            var seoField = await _fieldRepository.FindByNameAsync(SeoFieldNames.FieldName);
            var titleField = await _fieldRepository.GetAsync(SiteTestData.TitleFieldId);
            var bodyField = await _fieldRepository.GetAsync(SiteTestData.BodyFieldId);

            var contentType = await _contentTypeManager.CreateAsync(
                SiteTestData.BlogPageId, $"seo-body-{Guid.NewGuid():N}", "SEO type with body",
                fields: new[]
                {
                    new ContentTypeField(titleField.Id, order: 0),
