...
/// </summary>
public class SiteRouteResolver_Tests : SitesEntityFrameworkCoreTestBase
{
    ...
    public async Task Should_Resolve_Single_Page()
    {
        var match = await WithUnitOfWorkAsync(() =>
            _resolver.ResolveAsync("/about", SitesTestData.EnglishCulture));
    ...
    {
        var match = await WithUnitOfWorkAsync(() =>
            _resolver.ResolveAsync("/", SitesTestData.EnglishCulture));

        match.Kind.ShouldBe(RouteMatchKind.ContentOfPage);
        match.Page!.Id.ShouldBe(SitesTestData.HomePageId);
    ...
    {
        var match = await WithUnitOfWorkAsync(() =>
            _resolver.ResolveAsync("/blog", SitesTestData.EnglishCulture));
    ...
    {
        var match = await WithUnitOfWorkAsync(() =>
            _resolver.ResolveAsync("/blog/my-trip", SitesTestData.EnglishCulture));
    ...
    {
        var match = await WithUnitOfWorkAsync(() =>
            _resolver.ResolveAsync("/blog/summer-photos", SitesTestData.EnglishCulture));

        match.Kind.ShouldBe(RouteMatchKind.Content);
        match.Page!.Id.ShouldBe(SitesTestData.BlogPageId);
        match.ContentType!.Id.ShouldBe(SitesTestData.PostGalleryTypeId);
    }
    ...
    {
        var match = await WithUnitOfWorkAsync(() =>
            _resolver.ResolveAsync("/news/2026/07/launch", SitesTestData.EnglishCulture));

        match.Kind.ShouldBe(RouteMatchKind.Content);
        match.Page!.Id.ShouldBe(SitesTestData.NewsPageId);
        match.Content!.Slug.ShouldBe(SitesTestData.NewsSlug);
    }
    ...
    {
        var match = await WithUnitOfWorkAsync(() =>
            _resolver.ResolveAsync("/news/launch", SitesTestData.EnglishCulture));
    ...
    {
        var match = await WithUnitOfWorkAsync(() =>
            _resolver.ResolveAsync("/blog/draft-post", SitesTestData.EnglishCulture));
    ...
    {
        var match = await WithUnitOfWorkAsync(() =>
            _resolver.ResolveAsync("/blog/draft-post", SitesTestData.EnglishCulture, includeUnpublished: true));

        match.Kind.ShouldBe(RouteMatchKind.Content);
        match.Content!.Slug.ShouldBe(SitesTestData.DraftSlug);
    }
    ...
    {
        var match = await WithUnitOfWorkAsync(() =>
            _resolver.ResolveAsync("/blog/my-trip", SitesTestData.ChineseCulture));
    ...
    {
        var match = await WithUnitOfWorkAsync(() =>
            _resolver.ResolveAsync("/about", SitesTestData.ChineseCulture));

        match.Kind.ShouldBe(RouteMatchKind.ContentOfPage);
        match.Content!.CultureName.ShouldBe(SitesTestData.ChineseCulture);
    ...
    {
        var match = await WithUnitOfWorkAsync(() =>
            _resolver.ResolveAsync("/no/such/path", SitesTestData.EnglishCulture));
